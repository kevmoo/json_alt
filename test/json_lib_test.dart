...
void _testStringify() {
  ...
  // Lists.
  Expect.equals('[]', json.encode([]));
  Expect.equals('[]', json.encode(List(0)));
  Expect.equals('[null,null,null]', json.encode(List(3)));
  ...
  Expect.equals('[4,"a"]', json.encode(const _ToJson([4, "a"])));
  Expect.equals(
      '[4,{"x":42}]',
      json.encode(const _ToJson([
        4,
        _ToJson({"x": 42})
      ])));

  _expectThrowsJsonError(() => json.encode([const _ToJson(_ToJson(4))]));
  _expectThrowsJsonError(() => json.encode([Object()]));
}

void _testStringifyErrors() {
  // Throws if argument cannot be converted.
  _expectThrowsJsonError(() => json.encode(_TestClass()));

  // Throws if toJson throws.
  _expectThrowsJsonError(() => json.encode(_ToJsoner("bad", throws: true)));

  // Throws if toJson returns non-serializable value.
  _expectThrowsJsonError(() => json.encode(_ToJsoner(_TestClass())));
...
class _ToJsoner {
  ...
  Object toJson() {
    if (throws) throw Exception('oops!');
    return returnValue;
  }
}
