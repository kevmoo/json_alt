...
void main() {
  test('parse', () {
    ...
    // Lists.
    Expect.listEquals([], json.decode(' [] ') as List);
    Expect.listEquals(["entry"], json.decode(' ["entry"] ') as List);
    Expect.listEquals([true, false], json.decode(' [true, false] ') as List);
    Expect.listEquals([1, 2, 3], json.decode(' [ 1 , 2 , 3 ] ') as List);

    // Maps.
    Expect.mapEquals({}, json.decode(' {} ') as Map);
    Expect.mapEquals(
        {"key": "value"}, json.decode(' {"key": "value" } ') as Map);
    Expect.mapEquals(
        {"key1": 1, "key2": 2}, json.decode(' {"key1": 1, "key2": 2} ') as Map);
    Expect.mapEquals({"key1": 1}, json.decode(' { "key1" : 1 } ') as Map);
  });

  group('invalid', () {
    void testString(String s) {
      test(s, () {
        expect(() => json.decode(s), throwsFormatException);
      });
    }
