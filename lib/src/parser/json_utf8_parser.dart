...
class JsonUtf8Parser<T> extends ChunkedJsonParser<List<int>, T> {
  final bool allowMalformed;

  JsonUtf8Parser(JsonReader<T> listener, this.allowMalformed) : super(listener);

  @override
  int getChar(int index) => chunk[index];
  ...
  @override
  void beginString() {
    buffer = _Utf8StringBuffer(allowMalformed);
  }

  @override
  void addSliceToString(int start, int end) {
    _Utf8StringBuffer buffer = this.buffer as _Utf8StringBuffer;
  ...
  @override
  String endString() {
    var buffer = this.buffer as _Utf8StringBuffer;
    this.buffer = null;
...
class _Utf8StringBuffer {
  ...
  List<int> buffer = Uint8List(INITIAL_CAPACITY);
  ...
  void addCharCode(int char) {
    ...
    if (partialState != NO_PARTIAL) {
      if (allowMalformed) {
        ...
        throw const FormatException("Incomplete UTF-8 sequence");
      }
    }
  ...
  void _to16Bit() {
    ...
    if ((length + INITIAL_CAPACITY) * 2 <= buffer.length) {
      ...
      newBuffer = Uint16List.view((buffer as Uint8List).buffer);
    } else {
      int newCapacity = buffer.length;
      if (newCapacity - length < INITIAL_CAPACITY) {
        newCapacity = length + INITIAL_CAPACITY;
      }
      newBuffer = Uint16List(newCapacity);
    }
    ...
    while (position < end) {
      ...
      if ((char & MASK_CONTINUE_TAG) == CONTINUE_TAG) {
        ...
        if (allowMalformed) {
          ...
          throw FormatException("Invalid UTF-8 byte: $char", utf8, position);
        }
      }
      index = length;
      capacity = buffer.length;
  ...
  String toString() {
    if (partialState != NO_PARTIAL) {
      if (allowMalformed) {
        ...
        List source = Uint8List(seenByteCount);
