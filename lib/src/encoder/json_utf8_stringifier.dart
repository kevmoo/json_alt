...
typedef _AddChunk = void Function(Uint8List list, int start, int end);
...
class JsonUtf8Stringifier extends JsonStringifier {
  ...
  static void stringify(
    ...
    if (indent != null) {
      ...
      stringifier =
          JsonUtf8Stringifier(toEncodable, writer, bufferSize, addChunk);
    }
  ...
  void writeString(String characters) {
    writeStringSlice(characters, 0, characters.length);
  }
    ...
    for (var i = start; i < end; i++) {
      ...
      if (char <= 0x7f) {
        ...
        if ((char & 0xFC00) == 0xD800 && i + 1 < end) {
          // Lead surrogate.
          var nextChar = characters.codeUnitAt(i + 1);
